...
export default function ManagerRegistration({navigation}) {
    ...
    async function registerUser(){
        ...
        try{
            const snapShot = await managers.where('email', '==', registrationData.email).get()
            ...
            const result = await managers.add(user)
        } catch(e) {
            Alert.alert(e)
        }
        ...
        <View style={customerStyles.bodyContainer}>
            <View style={customerStyles.bodyInner}>
                <View style={{width: '100%', marginBottom: 16}}>
                    <Text style={[customerStyles.bodyHeadingMain, {color: colors.primary}]}>REGISTER AS MANAGER</Text>
                </View>
                <View>
